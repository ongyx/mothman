...
Copyright 2018 Raymond Velasquez, 2020 Ong Yong Xin
...
__author__ = "Raymond Velasquez <at.supermamon@gmail.com>, Ong Yong Xin"
__version__ = "0.4.1"
...
class DpkgScanpackages:
    def __init__(
        self, binary_path, multiversion=None, packageType=None, arch=None, output=None
    ):
        self.binary_path = binary_path
    ...
    def __get_packages(self):
        ...
        for fname in files:
            ...
            if self.multiversion == True:
                ...
                # find if package is already in the list
                matchedItems = [
                    (index, pkg)
                    for (index, pkg) in enumerate(self.packageList)
                    if self.packageList
...
def print_error(msg):
    ...
    COLOR_BOLD = "\033[1m"
    print(
        "{0}:{1}{2} error{3}: {4}".format(
            script_name, COLOR_RED, COLOR_BOLD, COLOR_RESET, msg
        )
    )
    print("")
